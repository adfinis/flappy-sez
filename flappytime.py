...
def send_message(message):
    ...
    requests.get("http://localhost:8089/set-text/%s" % message)
...
def is_own_message():
    ...
    current_msg = (
        requests.get("http://localhost:8089/get-text").content.decode("utf-8").strip()
    )
    try:
...
def lights_off():
    if "--pretend" in sys.argv:
        ...
        return
    requests.get("http://localhost:8089/light-off")
